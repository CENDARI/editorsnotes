...
# CENDARI : removed 
    ...
    symlink_packages()
    collect_static()
    local('if [ ! -d rdfstore ]; then mkdir rdfstore; fi')
    local('chmod -R a+rwX rdfstore')
...
def watch_static():
    ...
    class ChangeHandler(FileSystemEventHandler):
        ...
        def on_any_event(self, event):
            ...
            # CENDARI :removed sys.stdout.write(green('Finished\n\n'))
            self.last_collected = datetime.datetime.now()
...
def install_node_packages():
    ...
    with lcd(PROJ_ROOT):
        local('npm install')
    # to be removed ASAP
    with lcd("{}/node_modules/.bin/".format(PROJ_ROOT)):
        if not os.path.exists('lessc'):
            if os.path.exists('/usr/local/bin/lessc'):
                local('ln -s -f /usr/local/bin/lessc lessc')
            elif os.path.exists('/usr/bin/lessc'):
                local('ln -s -f /usr/bin/lessc lessc')
            else:
