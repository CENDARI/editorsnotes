...
var IIPMooViewer = new Class({
   ...
   */
  changeImage: function( image, loadoptions ) {
    ...
    this.images = [{ src:image, sds:"0,90", cnt:(this.viewport&&this.viewport.contrast!=null)? this.viewport.contrast : null } ];
      ...
      if ( loadoptions ) {
	  this.max_size = loadoptions.size;
	  this.tileSize = loadoptions.tiles;
	  this.num_resolutions = loadoptions.resolutions;
	  this.reload();
	  if( this.navigation )
	      this.navigation.setImage(
		  this.protocol.getThumbnailURL(
		      this.server, image, this.navigation.size.x));
      }
      else {
